...
HRESULT __stdcall hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (CheatFunctions::ShowPlayerInformationBoxNow && CheatFunctions::CheatInitEntirelyCompleted)
	{
		...
		if (ImGui::BeginTable("PlayerInfoBoxTable", 4, ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_SizingFixedFit))
		{
			...
			ImGui::TableSetColumnIndex(2);
			ImGui::Text("External IP Address");

			ImGui::TableSetColumnIndex(3);
			ImGui::Text(CheatFunctions::PlayerInfoBoxDataObject.ExternalIPAddress.c_str());
			...
			ImGui::TableSetColumnIndex(2);
			ImGui::Text("Internal IP Address");

			ImGui::TableSetColumnIndex(3);
			ImGui::Text(CheatFunctions::PlayerInfoBoxDataObject.InternalIPAddress.c_str());
