...
bool GUI::Option(std::string option, std::string InformationText, int BitFlags)
{
	...
	if (OnCurrent)
	{
		...
		if (!CheatFeatures::HideSelectableInformationBox && !(BitFlags & SELECTABLE_HIDE_INFO_BOX))
		{
			...
			if (GUI::CurrentOptionIsSavable) { DrawTextInGame("Save: " + CheatFunctions::VirtualKeyCodeToString(Controls::SaveSelectableKey), TextColorAndFont, { SelectableInfoBoxX + 0.07f, SelectableInfoBoxY - 0.033f }, { 0.30f, 0.30f }, false); }
		}
		...
		if (GUI::currentOption == GUI::optionCount)
		{
			...
			if (!CheatFeatures::HideVehicleInfoAndPreview)
			{
				...
				if (guiX < 0.71f)
				{
					...
					DrawTextInGame("Seats: " + std::to_string(VEHICLE::GET_VEHICLE_MODEL_NUMBER_OF_SEATS(VehicleHash)), TextColorAndFont, { guiX + 0.111f, guiY + 0.025f }, { 0.45f, 0.30f }, false);
				...
				else
				{
					...
					DrawTextInGame("Seats: " + std::to_string(VEHICLE::GET_VEHICLE_MODEL_NUMBER_OF_SEATS(VehicleHash)), TextColorAndFont, { guiX - 0.262f, guiY + 0.025f }, { 0.45f, 0.30f }, false);
