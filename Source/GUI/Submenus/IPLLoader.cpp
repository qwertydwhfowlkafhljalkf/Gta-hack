...
void GUI::Submenus::IPLLoader()
{
	...
	if (GUI::StringVector("GTA:O Map Data", { "Load", "Unload" }, LoadOrUnloadMPData, "Load or unload GTA Online map data - required to load some IPL's in SP"))
	{
	...
	if (GUI::StringVector("North Yankton", { "Load", "Unload" }, IPLLoaderNorthYankton, ""))
	{
	...
	if (GUI::StringVector("Dignity Heist Yacht", { "Load", "Unload" }, IPLLoaderDignityHeistYacht, ""))
	{
	...
	if (GUI::StringVector("Destroyed Hospital", { "Load", "Unload" }, IPLLoaderDestroyedHospital, ""))
	{
	...
	if (GUI::StringVector("Jewelry Store", { "Load", "Unload" }, IPLLoaderJewelryStore, ""))
	{
	...
	if (GUI::StringVector("Morgue", { "Load", "Unload" }, IPLLoaderMorgue, ""))
	{
	...
	if (GUI::StringVector("Cargoship", { "Load", "Unload" }, IPLLoaderCargoship, ""))
	{
	...
	if (GUI::StringVector("Aircraft Carrier", { "Load", "Unload" }, IPLLoaderAircraftCarrier, "You must load the GTA:O map data (see option above) for this IPL to work in SP"))
	{
